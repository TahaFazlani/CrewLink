...
export class TenantScope {
  ...
  /**
   * Default query path. Always filters by the authenticated member's local.
   * Entities with `local_id` get `WHERE local_id = :localId`.
   * `locals` is scoped by `id`. `announcement_recipients` joins `announcements`.
   */
  forEntity<T extends ObjectLiteral>(
    ...
    if (!hasLocalId) {
      throw new Error(
        `TenantScope.forEntity: ${meta.tableName} is not tenant-scoped; use unsafeUnscopedQuery`,
      );
    }

    qb.andWhere(`${alias}.localId = :tenantLocalId`, { tenantLocalId: localId });
    return qb;
  }

  /** Opt-out. Do not call from HTTP handlers. */
  unsafeUnscopedQuery<T extends ObjectLiteral>(
    entity: EntityTarget<T>,
    alias: string,
  ): SelectQueryBuilder<T> {
  ...
  private async loadUnscopedForAuthzLog<T extends ObjectLiteral>(
    ...
    const meta = this.dataSource.getMetadata(entity);
    const qb = this.unsafeUnscopedQuery(entity, alias).andWhere(
      `${alias}.id = :id`,
      { id },
    );
    if (meta.tableName === 'announcement_recipients') {
  ...
  private resourceLocalId(row: ObjectLiteral): string {
    ...
    if (typeof row.id === 'string' && !('localId' in row) && !row.announcement) {
      return row.id;
    }
